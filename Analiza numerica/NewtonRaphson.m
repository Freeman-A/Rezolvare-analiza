function [xaprox, iteratii, err_abs, err_rel, err_rez] = NewtonRaphson(f, x0aprox, TOL, g, a, b)
...
xaprox(n) = x0aprox;
n = 2;
xaprox(n) = xaprox(1) - f(xaprox(1)) / g(xaprox(1));
err_abs(n) = eroarea_abs(n);
err_rel(n) = eroarea_rel(n);
err_rez(n) = eroarea_rez(n);

while err_abs(n)>TOL && err_rel(n)>TOL && err_rez(n)>TOL
    n = n + 1;
...
%Special function which returns an error.
    ...
    function output = eroarea_abs(n)
        output = abs(xaprox(n) - xaprox(n-1));
    end
    
    function output = eroarea_rel(n)
        output = abs(xaprox(n) - xaprox(n-1)) / abs(xaprox(n));
    end
